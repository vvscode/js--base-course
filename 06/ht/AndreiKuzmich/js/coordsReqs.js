...
var getLatLngByXhr = function () {
  return new Promise(function (resolve) {
    ...
    GOOGLE_API_KEY = 'AIzaSyBfCL7i-AyKlw0tHfaIVWCfkC7sudtWvLM';
    url = `https://maps.googleapis.com/maps/api/geocode/json?address=${addr1 || 'Lida'}&key=${GOOGLE_API_KEY}`;
...
var getLatLngByFetch = function (addr) {
  var addr1 = document.querySelector('#find').value;
  GOOGLE_API_KEY = 'AIzaSyBfCL7i-AyKlw0tHfaIVWCfkC7sudtWvLM';
  url = `https://maps.googleapis.com/maps/api/geocode/json?address=${addr || addr1 || 'Lida'}&key=${GOOGLE_API_KEY}`;
  return fetch(url)
    .then(function (res) {
      return res.json();
    })
...
function getCitiByCoords(lat, lng) {
  GOOGLE_API_KEY = 'AIzaSyBfCL7i-AyKlw0tHfaIVWCfkC7sudtWvLM';
  url = `https://maps.googleapis.com/maps/api/geocode/json?latlng=${`${lat},${lng}`}&key=${GOOGLE_API_KEY}&language=en`;
  return fetch(url)
    .then(function (res) {
      return res.json();
    })
