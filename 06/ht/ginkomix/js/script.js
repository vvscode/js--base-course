...
//    this.XHR = this.XHR.bind(this); Есть ли смысл?
//    this.fetch = this.fetch.bind(this); 
    
}

Queries.prototype.XHR = function(cord) {
    var XHR = ("onload" in new XMLHttpRequest()) ? XMLHttpRequest : XDomainRequest,
        xhr = new XHR(),
        self = this;

    xhr.open('GET', 'http://cors-proxy.htmldriven.com/?url=https://api.darksky.net/forecast/'+self.key+'/'+cord, true);// Как мне тут получить key, совсем не понимаю
    xhr.onload = function() {
...
Queries.prototype.fetch = function(cord) {
    fetch('http://cors-proxy.htmldriven.com/?url=https://api.darksky.net/forecast/'+this.key+'/'+cord)
...
Map.prototype = Object.create(Queries.prototype);
...
Map.prototype.changeHash = function(cord) {
    ...
    for(var i = 0;i < radio.length;i ++) {
        if(radio[i].checked) {
            if(radio[i].id === 'xhr') {
                ...
                eb.on('changeHash',map.XHR);
                eb.off('changeHash',map.fetch);
            ...
            if(radio[i].id === 'fetch') {
                ...
                eb.off('changeHash',map.XHR);
                eb.on('changeHash',map.fetch);
