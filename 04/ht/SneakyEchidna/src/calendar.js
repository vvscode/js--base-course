let lStorage = {
  setData: (key, value) => new Promise((resolve, reject) => {
 ...
 */
  function Calendar(options) {
    this.options = Object.assign({allowChangeMonth: true, className: 'calendar'}, options);
 ...
 */
  ...
  for (let i = 0; i < days.length; i++) {
      let key = options.el + '.' + days[i].innerHTML.split(' ')[0] + '.' + thisMonth + '.' + thisYear;
...
function addNote(target) {
  ...
  askUser.setNote(thisElement).then((result) => {
    storage.setData(key, result).then((result) => {
      ...
      thisElement.onclick = btnDeleteClickHandler.bind(thisElement, result[0]);
    });
});
}
/**
...
function drawNote(dayElement, result) {
  ...
  if (options.allowRemove) {
    note = dayElement.innerHTML.split(' ')[0]
    + ' <div class="divNote"><label class="lblNote">'
    + result + '</label><button class="btnDelete">x</button ></div >';
  } else {
    note = dayElement.innerHTML.split(' ')[0]
    + ' <div class="divNote"><label class="lblNote">'
    + result + '</label></div >';
