...
function drawCalendar(year, month, htmlEl) {
    let date = new Date(year, month-1),
        today = date.getDate(),
        firstDay = ((new Date(date.getFullYear(), date.getMonth(), 1)).getDay() === 0) ? 7 : (new Date(date.getFullYear(), date.getMonth(), 1)).getDay(), // Определения дня недели 1-го числа
        ...
        calendar = '<table><caption>' + monthRU[date.getMonth()] + ' ' + year + '</caption>' + '<tr><th>' + dayRU[1] + '</th><th>' + dayRU[2] + '</th><th>' + dayRU[3] + '</th><th>' + dayRU[4] + '</th><th>' + dayRU[5] + '</th><th>' + dayRU[6] + '</th><th>' + dayRU[0] + '</th></tr>',
        dayInMonth = 33 - new Date(date.getFullYear(), date.getMonth(), 33).getDate(), // Определение количества дней в месяце, с 32 в Сафари баг
        countDay = 1;
