...
describe("spiral", function () {
    ...
    it(" сохраняет неизмененным входящий массив ", function () {
        let inputArr = [[1, 2], [3, 4], [5, 6], [7, 8]];
        let copyArr = Object.assign( inputArr);
        let a = spiral(inputArr);
console.log(inputArr);
console.log(copyArr);
        let b = [1, 2, 4, 6, 8, 7, 5, 3];
        return assert.deepEqual(a, b) && assert.deepEqual(a, b)(inputArr, copyArr);
    });
});
...
describe("quadraticEquation", function () {
    ...
    it("1, 6, 1- правильный результат ( 2 корня)", function () {
        ...
        return assert.isOk(isDeepEqual(a, b) || isDeepEqual(a, c));
    });
});
